...
f3a = pd.read_csv("source_data/Figure3a_tree_aligned_interval_presence.tsv", sep="\t")
f3b = pd.read_csv("source_data/Figure3b_full_atlas_interval_prevalence.tsv", sep="\t")
f3c = pd.read_csv("source_data/Figure3c_bioproject_stratified_statistics.tsv", sep="\t")
f3d = pd.read_csv("source_data/Figure3d_representative_gene_order.tsv", sep="\t")
...
def build_fig3_v3():
    ...
    for tip in order:
        ...
        if src.get(nm) == "local":
            ...
            ax_lab.text(0.08, y[tip], samp[nm], fontsize=5.4, fontweight="bold", ha="left", va="center", color=col)
        else:
    ...
    for cx, b in zip(centers, BLOCK_ORDER):
        ax_heat.text(cx, n+0.4, SHORT[b], rotation=90, ha="center", va="bottom",
                     fontsize=5.6, color=block_color[b])
    leg_a = [
    ...
    axd.text(0.0, -0.06, "coloured, stable family;  grey, intervening ORF;  row scales independent",
             transform=axd.transAxes, fontsize=5.4, color=C["muted"], ha="left", va="top")
